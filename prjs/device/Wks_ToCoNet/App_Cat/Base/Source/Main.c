...
void cbToCoNet_vRxEvent(tsRxDataApp *pRx) {
  ...
  for(i = 0;i < MAX_CATS;i ++){
    if(sAddrSeq[i].u32CatAddr == sPkt.u32CatAddr){
    ...
    if(sAddrSeq[i].u32CatAddr == 0 &&
      ...
      sAddrSeq[i].u32CatAddr = sPkt.u32CatAddr;
      sAddrSeq[i].u8SeqPrev = pRx->u8Seq;
      bSendPkt = TRUE;
      break;
    }
  ...
  if(bSendPkt){
    ...
    vfPrintf(&sSerStream, "[PKT Ad:%04x,Ln:%03d,Seq:%03d,Lq:%03d,Tms:%05d,",
	     ...
	     pRx->u8Lqi,
	     pRx->u32Tick & 0xFFFF);
    vfPrintf(&sSerStream, "Typ:%02x,X:%03d,Y:%03d,Z:%03d,CatAd:%04x,CatLq:%03d]" LB,
	     sPkt.u8Type, sPkt.u16Dx, sPkt.u16Dy, sPkt.u16Dz, sPkt.u32CatAddr, sPkt.u8CatLqi);
...
static void vProcessEvCore(tsEvent *pEv, teEvent eEvent, uint32 u32evarg) {
  ...
  case E_STATE_IDLE:
	...
	vfPrintf(&sSerStream, "*** App_Cat_Base %d.%02d-%d ***" LB, VERSION_MAIN, VERSION_SUB, VERSION_VAR);
